...
- Issue-requested validation was not fully runnable in this checkout:
  `markdownlint` was not installed, `npx --no-install markdownlint-cli2`
  attempted registry access and was blocked by the restricted runner
  environment, and `scripts/release/check_changelog.py` is absent.
...
- `docs/validation/local_codex_runner/issue-125/gate_report.json` records
  `type:docs` gate status `PASS` via `python -m compileall scripts`.
...
- Formatting and changelog validation coverage is incomplete until
  `markdownlint` and `scripts/release/check_changelog.py` are available.
