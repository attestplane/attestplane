...
- Ran `cd sdk/python && .venv/bin/ruff check src tests`.
- Ran `cd sdk/python && .venv/bin/mypy`.
- Ran `cd sdk/python && .venv/bin/pytest -q tests/conformance/test_verifier_conformance.py tests/cli/test_main.py tests/cli/test_verify_errors.py tests/cli/test_verify_cli_deterministic_json.py`.
...
- Test result: `35 passed` for the issue-focused SDK slice, `41 passed` for the broader local slice.
