...
- `taxonomy_version` mirrors the proof bundle's
  `chain_metadata.evidence_taxonomy_version` field and is surfaced
  identically by `verify --json`, `verify --explain`, and the SDK verifier
  result object.
