...
- The `latest` alias redirects to the latest suffix-free stable release.
- The `releases/vX.Y.Z/` path is the exact release snapshot for a stable tag such as
  `v1.5.0`.
- The `lines/vX.Y/` path is the stable line alias for a release line such as `v1.5`.
