...
| 1.0 GA | 完整 SLA | 目标发布日期 2026-08-15。下文的响应时限自该版本起仅前向适用 / 向前生效。 |
...
`security@attestplane.com` 的专用 GPG 密钥。在该日期之前，项目不在
流通任何 GPG 密钥；在通过下列渠道正式发布之前，不要信任任何自称是
...
以上时限自 v1.0 GA（目标 2026-08-15）起生效。Pre-GA / GA 之前阶段，
我们的响应工作为尽力而为 / best-effort，不对外构成承诺。
...
| Audit log API | JSON Auditor API — 查询、追加与校验端点（M5 表面） |
| SDK helpers | FastAPI / Express / NestJS / Django 框架集成辅助库（M5 表面） |
...
| AT-02 | **重放攻击** — 一条合法的、已签名的审计事件或 attestation 被重放至错误顺序中，伪造出具误导性的审计链。 | High | 链中已强制使用序号与时间戳；nonce 策略计划于 M5。 |
...
| AT-04 | **聚合污染（M7 差分隐私）** — 客户端侧被操纵的输入污染了提交给监管机构的隐私保护聚合报告。 | High | 计划在 M7 C1 的客户端 DP 架构中缓解；并对输入施加边界校验。 |
...
EU Cyber Resilience Act 将于 **2027-12-11** 起生效。Attestplane 是一个由商业实体（Attestplane Pte. Ltd.）维护的开源项目。开源项目守护人与商业制造者在 CRA 下的义务区分，目前处于持续的法律观察之中。

| 检查点 | 目标日期 | 行动 |
|------------|-------------|--------|
| 法律站位评估（OSS steward 对比 commercial manufacturer 分类） | 2027-Q1 | 由创始人（China-licensed compliance lawyer）对 CRA recitals 与实施细则进行自我评估 |
...
本 SECURITY.md 中部分章节与 **AIA-12 *aligned* profile**（AIA-12 一致性档案，详见英文原文 `docs/spec/aia-12-aligned-profile.md`）的范畴相邻；本中文译本不修改该规范文档，仅在路径上加以引用。

---

## 荣誉墙 / Bug Bounty

本阶段（M5 之前的 alpha 阶段）暂无正式的金钱性 Bug Bounty 计划。提交了在范围内、且经验证合法漏洞的研究者将会：
...
- **拥有数据库超级用户权限的内部威胁。** 技术控制（append-only 授权、哈希链、RFC-3161 anchoring）只能减小但无法消除来自高权限数据库管理员的风险。组织性控制 — 最小权限访问、schema 变更的双人审批、以及将审计日志导出到独立存储 — 是必需的互补缓解。
...
> **声明：** 本文件为 SECURITY.md 的中文译本，并 **不是合规认证**、**不是法律意见**；亦不构成 Attestplane 项目对运营方所部署环境合规性的任何积极保证。译本如与英文原版存在歧义，以英文原版为准。

*本文件最近一次审阅：2026-05-20。下次计划审阅：2027-Q1（CRA 法律站位检查点）。*
