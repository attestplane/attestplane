...
source_commit: 499958cec6939662c74a7f3301d1a62c1c822d0c
source_sha: 499958cec6939662c74a7f3301d1a62c1c822d0c
...
> **Source / 英文原文：** [SECURITY.md](./SECURITY.md) @ `499958ce`
...
当 `SECURITY.md` 有变更时，请按以下顺序维护中文译本：

1. 先合入英文版的安全修订，紧急安全信息不要因为翻译未完成而被阻塞。
2. 同步更新 `SECURITY_zh.md` 的正文和 frontmatter 里的 `source_commit`。
3. 运行 `python scripts/docs/check_translation_drift.py`；在 `drift_grace_days` 窗口内只会发出 warning，超过窗口后会失败。
4. 如果中文译本暂时无法同步，保留英文版为权威来源，并在 grace period 内补齐译文和元数据。
5. 若中英文存在歧义，以英文版 `SECURITY.md` 为准。
