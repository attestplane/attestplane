...
def test_integration_payload_is_non_authoritative(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run(argv: list[str], *, timeout: int = 20) -> tuple[bool, str]:
        ...
        if argv[:3] == ["git", "ls-remote", "origin"] and argv[-1] == "refs/tags/v0.1.5-alpha":
            return True, "tag123\trefs/tags/v0.1.5-alpha"
        if argv[:3] == ["git", "status", "--short"]:
            return True, ""
        if argv[:3] == ["gh", "release", "view"]:
            return True, json.dumps({"tagName": "v0.1.5-alpha", "isPrerelease": True, "assets": []})
    ...
    monkeypatch.setattr(alpha_train_integrations.shutil, "which", lambda name: f"/bin/{name}")
    monkeypatch.setattr(alpha_train_integrations, "run_capture", fake_run)
    monkeypatch.setattr(alpha_train_integrations, "pypi_version_exists", lambda version: True)
    ...
    def fake_urlopen(request: object, timeout: int = 20) -> FakeResponse:
        ...
        if "api.linear.app/graphql" in url:
            return FakeResponse({"data": {"viewer": {"id": "viewer-1", "name": "Attestplane"}, "organization": {"id": "org-1", "name": "Attestplane"}}})
    ...
    monkeypatch.setattr(alpha_train_integrations.urllib.request, "urlopen", fake_urlopen)

    payload = alpha_train_integrations.build_status_payload("v0.1.5-alpha", state_path=state_file)
...
def test_integration_reports_write_json_and_markdown(monkeypatch, tmp_path: Path) -> None:
    state_file = tmp_path / "state.json"
    payload: dict[str, Any] = {
        ...
        "linear": {"available": True, "configured": True, "workspace_observed": True, "viewer_observed": True},
        "sentry": {"available": True, "configured": True, "organization_observed": True, "unresolved_issue_count": 2},
    ...
    monkeypatch.setattr(alpha_train_integrations, "build_status_payload", lambda release, state_path: payload)
    ...
    assert json.loads(json_path.read_text(encoding="utf-8"))["schema"] == "attestplane_alpha_integration_status.v1"
...
def test_integration_payload_marks_missing_optional_workflow_surfaces(monkeypatch, tmp_path: Path) -> None:
    state_file = tmp_path / "state.json"
    db_path = tmp_path / "state.sqlite"
    with sqlite3.connect(db_path) as db:
        db.execute(
    ...
    monkeypatch.setattr(alpha_train_integrations.shutil, "which", lambda name: f"/bin/{name}")
    monkeypatch.setattr(alpha_train_integrations, "run_capture", lambda argv, timeout=20: (False, "missing"))
    monkeypatch.setattr(alpha_train_integrations, "pypi_version_exists", lambda version: False)
