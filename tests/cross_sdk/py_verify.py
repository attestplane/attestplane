...
def diff_section(name: str, py_entries: list[dict[str, str]], ts_entries: list[dict[str, str]]) -> list[str]:
    ...
    for cid in sorted(set(py_idx) & set(ts_idx)):
        py = py_idx[cid]
        ts = ts_idx[cid]
        if py["canonical_b64"] != ts["canonical_b64"] or py["hash_hex"] != ts["hash_hex"]:
            failures.append(
...
def main() -> int:
    ...
    failures: list[str] = []
    failures += diff_section("canonical_text", py_emit["canonical_text"], ts_reemit["canonical_text"])
    failures += diff_section("canonical_json", py_emit["canonical_json"], ts_reemit["canonical_json"])
