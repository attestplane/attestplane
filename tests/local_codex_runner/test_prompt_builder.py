...
def test_prompt_contains_issue_and_red_lines(tmp_path: Path) -> None:
    task = IssueTask(9, "Claim gap", "Body with token=secret123", "https://issue", ["auto-codex-approved", "claim-safety"])
...
def test_pr_body_contains_fixes(tmp_path: Path) -> None:
    ...
    body = PromptBuilder(tmp_path).build_pr_body(task, validation="pytest", evidence="docs/validation/x", residual_risks="None")
