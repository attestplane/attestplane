...
def test_run_issue_can_fetch_open_issue_without_approval_label(monkeypatch, tmp_path: Path) -> None:
    class FakeGH:
        def __init__(self, dry_run):
            self.commands_run = []
...
def test_run_issue_accepts_in_progress_issue_without_local_mapping(monkeypatch, tmp_path: Path) -> None:
    class FakeGH:
        def __init__(self, dry_run):
            self.commands_run = []
        ...
        def list_issues(self, repo, label, limit):
            ...
            assert label is None
            return [IssueTask(8, "Fix in-progress gap", "body", "url", ["codex-in-progress"])]
