...
def test_verify_explain_mentions_quarantine_cause(
    capsys: pytest.CaptureFixture[str],
) -> None:
    quarantine_fixture = ROOT / "fixtures" / "anchoring" / "quarantine_timeout.att"
    rc = main(["verify", "--explain", str(quarantine_fixture)])
    captured = capsys.readouterr()

    assert rc == 0
    assert "anchoring.status=quarantine" in captured.out
    assert "anchoring.cause=tsa request timeout" in captured.out
