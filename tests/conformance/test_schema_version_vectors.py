...
def test_schema_version_additive_optional_fixture_documents_its_rule() -> None:
    readme = SCHEMA_VERSION_DIR / "additive_with_unknown_field_ok" / "README.md"
    text = readme.read_text(encoding="utf-8").lower()

    assert "forward-compatible" in text
    assert "additive optional" in text
    assert "critical_future_field" in text
    assert "#185" in text
    assert "#173" in text
    assert "#210" in text
