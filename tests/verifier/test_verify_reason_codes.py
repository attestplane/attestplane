...
def test_verify_reason_code_taxonomy_version_pin_matches_when_required() -> None:
    result = verify_proof_bundle(_signed_bundle(), require_taxonomy_version=1)
...
def test_verify_reason_code_taxonomy_version_missing_is_primary_for_pinned_requirement() -> (
    None
):
    bundle = _signed_bundle()
    del bundle["chain_metadata"]["evidence_taxonomy_version"]

    result = verify_proof_bundle(bundle, require_taxonomy_version=1)
...
def test_verify_reason_code_taxonomy_version_unsupported_is_primary_for_pinned_requirement() -> (
    None
):
    bundle = _signed_bundle()
