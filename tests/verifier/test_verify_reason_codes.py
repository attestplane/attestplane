...
def _quarantined_bundle() -> dict:
    bundle = _bundle()
    bundle["anchoring"] = {
        "anchoring_status": "quarantined",
        "quarantine_reason": "tsa provider returned malformed token",
    }
    return bundle
...
def test_verify_reason_code_anchoring_status_verified_when_anchor_ref_present() -> None:
    bundle = _bundle()
    bundle["chain_metadata"]["anchor_ref"] = "anchor://chain/0"
...
def test_verify_reason_code_anchoring_status_quarantined_from_bundle_annotation() -> (
    None
):
    result = verify_proof_bundle(_quarantined_bundle())

    assert result.ok is True
    assert result.anchoring_status == "quarantined"
    assert result.quarantine_reason == "tsa provider returned malformed token"
