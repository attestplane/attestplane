...
def test_verify_reason_code_taxonomy_version_mismatch_is_primary_for_pinned_bundle() -> (
    None
):
    result = verify_proof_bundle(_bundle(), require_taxonomy_version=2)
    ...
    assert result.metadata_reason is None
    assert result.taxonomy_version_ok is False
    assert "required taxonomy_version=2" in (result.taxonomy_version_reason or "")
