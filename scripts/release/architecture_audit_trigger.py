...
from scripts.opus_consult.telemetry import compute_prompt_hash, emit_plan_consult_event  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    if decision.should_upload_artifact:
        ...
        if decision.should_open_issue:
            ...
            _consult_duration_ms = int((time.monotonic() - _consult_t0) * 1000)
            _prompt_hash = compute_prompt_hash(issue_body)
            emit_plan_consult_event(
                milestone=str(manifest["milestone_tag"]),
                plan_level=str(manifest.get("plan_level", "daily")),
                anchor=str(manifest.get("anchor_tag") or "repository start"),
                head_sha=str(manifest["head_sha"]),
                duration_ms=_consult_duration_ms,
                exit_code=1 if accepted_plan.source == "deterministic-template" else 0,
                fallback_used=(accepted_plan.source == "deterministic-template"),
