...
def validate_manifest(
    manifest: dict[str, Any], path: Path, repo_root: Path
) -> list[str]:
    errors: list[str] = []
    ...
    for backend in backends:
        ...
        for field in ("name", "status", "record_format", "compatibility_policy"):
            if not isinstance(backend.get(field), str) or not backend[field]:
                errors.append(
                    f"{path}: backend {backend.get('name')!r} missing {field}"
                )
        if backend.get("name") == "jsonl" and backend.get("multi_writer_safe") is True:
            errors.append(
                f"{path}: jsonl backend must not claim multi_writer_safe=true"
            )
        if backend.get("destructive_repair") not in {
            "not_implemented",
            "explicit_operator_only",
        }:
            errors.append(
                f"{path}: backend {backend.get('name')!r} has unsafe destructive_repair"
            )
    ...
    if migration.get("default") != "no_destructive_migration":
        errors.append(
            f"{path}: default migration policy must be no_destructive_migration"
        )
    ...
    issue_format = next(
        (
            item
            for item in formats
            if isinstance(item, dict) and item.get("name") == "storage_scan_issue.v1"
        ),
        None,
    )
    if not isinstance(issue_format, dict):
        ...
        if required_fields != {"kind", "line_no", "byte_offset", "detail"}:
            errors.append(
                f"{path}: storage_scan_issue.v1 required_fields must match implementation"
            )
    implementation_codes = implementation_issue_codes(
        repo_root / "sdk/python/src/attestplane/storage/jsonl.py"
    )
    ...
    for fixture in fixtures:
        ...
        if not isinstance(fixture.get("negative"), bool):
            errors.append(
                f"{path}: fixture {fixture_path_raw} missing boolean negative"
            )
        expected = fixture.get("expected_issue_code")
        if fixture.get("negative") is True and expected not in known_codes:
            errors.append(
                f"{path}: negative fixture {fixture_path_raw} has unknown expected_issue_code {expected!r}"
            )
        if fixture.get("negative") is False and expected is not None:
            errors.append(
                f"{path}: positive fixture {fixture_path_raw} must not set expected_issue_code"
            )

    expected = json.dumps(manifest, indent=2, sort_keys=True) + "\n"
    if path.read_text(encoding="utf-8") != expected:
        errors.append(
            f"{path}: JSON is not deterministic; sort keys with two-space indentation"
        )
    return errors
