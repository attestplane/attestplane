...
function hashJson(value: unknown): string {
  ...
  return createHash('sha256').update(JSON.stringify(sortDeep(value)), 'utf-8').digest('hex');
}
...
describe('LangFuseAdapter', () => {
  ...
  it('null input still emits hash of empty object', () => {
    const adapter = new LangFuseAdapter();
    const draft = adapter.translate({
      id: 'o', trace_id: 't', type: 'GENERATION', name: 'x', input: null,
    });
  ...
  it('user_id → SubjectRef', () => {
    const adapter = new LangFuseAdapter();
    const draft = adapter.translate({
      id: 'o', trace_id: 't', type: 'GENERATION', name: 'x', user_id: 'alice',
    });
  ...
  it('rejects non-Observation input', () => {
    const adapter = new LangFuseAdapter();
    expect(() =>
      adapter.translate({ id: 'o' } as unknown as LangFuseObservation),
    ).toThrow(AdapterTranslationError);
  });
  ...
  it('pure function — same input same output', () => {
    ...
    const obs: LangFuseObservation = {
      id: 'o', trace_id: 't', type: 'GENERATION', name: 'x',
      start_time: NOW, end_time: NOW, input: { a: 1 },
    };
