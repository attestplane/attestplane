...
def test_auditor_export_anchor_status_quarantine_round_trips() -> None:
    builder = ProofBundleBuilder(chain_id="q", producer_runtime="test", anchor_status="quarantine")
    bundle = builder.build()
    export = build_auditor_export(bundle)
    assert export["chain_summary"]["anchor_status"] == "quarantine"


def test_verify_proof_bundle_rejects_quarantine_anchor_status() -> None:
    builder = ProofBundleBuilder(chain_id="q", producer_runtime="test", anchor_status="quarantine")
    bundle = builder.build()
    result = verify_proof_bundle(bundle)

    assert result.ok is False
    assert "anchor_status" in (result.metadata_reason or "")
