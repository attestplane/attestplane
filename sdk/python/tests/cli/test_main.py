...
@pytest.mark.parametrize(
    ...
    [
        ("quarantine_timeout.att", "timeout"),
        ("quarantine_malformed_token.att", "malformed"),
        ("quarantine_cert_chain_mismatch.att", "chain"),
    ],
)
def test_verify_quarantine_fixtures_fail_closed(
    fixture_name: str,
    expected_reason: str,
    capsys: pytest.CaptureFixture[str],
) -> None:
    fixture = Path(__file__).resolve().parents[4] / "fixtures" / "anchoring" / fixture_name
    rc = main(["verify", str(fixture), "--json"])

    payload = json.loads(capsys.readouterr().out)
    assert rc == 1
    assert payload["result"] == "fail"
    assert payload["exit_code"] == 1
    assert payload["anchor_status"] == "quarantine"
    assert expected_reason in (payload["anchor_ref"] or "")
    assert payload["reasons"][0]["path"] == "/chain_metadata/anchor_status"
