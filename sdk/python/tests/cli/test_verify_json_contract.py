...
This locks the canonical JSON output surface and the versioned contract
fixture introduced by issue #408.
"""
...
GOLDEN_FIXTURE = CONFORMANCE_FIXTURES / "golden" / "verify_json_contract.json"
VERIFY_JSON_GOLDEN = GOLDEN_FIXTURE.read_text(encoding="utf-8")
...
def test_verify_json_pass_fixture_emits_fixed_schema(
    ...
    rc, out, payload, stderr = _run_verify(["verify", "--json", str(PASS_FIXTURE)], capsys)

    assert rc == VERIFY_JSON_EXIT_CODES["accept"]
    assert out == VERIFY_JSON_GOLDEN
    assert stderr == ""
    assert payload == json.loads(VERIFY_JSON_GOLDEN)
