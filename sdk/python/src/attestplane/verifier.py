...
def _taxonomy_version(metadata: dict[str, Any]) -> int | None:
    version = metadata.get("evidence_taxonomy_version")
    if isinstance(version, int):
        return version
    return None


def _taxonomy_version_reason(metadata_reason: str | None) -> VerifyReasonCodeV1 | None:
    if metadata_reason is None:
        return None
    if metadata_reason.startswith("chain_metadata.evidence_taxonomy_version"):
        return VERIFY_REASON_TAXONOMY_VERSION_MISMATCH
    return None
...
def _verification_reasons(
    *,
    ...
    if not metadata_ok:
        taxonomy_version_reason = _taxonomy_version_reason(metadata_reason)
...
def _verify_metadata_closure(
    ...
    *,
    ...
    if require_taxonomy_version is not None:
        taxonomy_version = _taxonomy_version(metadata)
        ...
        if taxonomy_version != require_taxonomy_version:
            return False, (
                f"chain_metadata.evidence_taxonomy_version={taxonomy_version!r} does not match "
                f"required taxonomy version {require_taxonomy_version!r}"
            )
