...
def _resolve_taxonomy_version(bundle: dict[str, Any] | None) -> int | None:
    """Return the bundle-declared taxonomy version when present.

    Legacy bundles may omit ``chain_metadata.evidence_taxonomy_version``.
    The verifier surfaces that as ``None`` rather than raising.
    """
...
class BundleVerificationResult:
    ...
    ``taxonomy_version`` mirrors the bundle's
    ``chain_metadata.evidence_taxonomy_version`` when present and is
    ``None`` for legacy bundles that do not declare the field.
    """
