...
def _verify_metadata_closure(
    ...
    *,
    ...
    anchor_status = metadata.get("anchor_status", "unanchored")
    if not isinstance(anchor_status, str):
        return False, "chain_metadata.anchor_status must be a string"
    if anchor_status not in _ALLOWED_ANCHOR_STATUSES:
        return False, (
            f"chain_metadata.anchor_status={anchor_status!r}; this verifier handles "
            f"statuses {sorted(_ALLOWED_ANCHOR_STATUSES)}"
        )
    if anchor_status == "quarantine":
        return False, "chain_metadata.anchor_status is quarantine"
