...
def verify_taxonomy_version_requirement(
    require_taxonomy_version: str | None,
) -> tuple[bool, str | None]:
    """Check an opt-in consumer taxonomy pin against the supported taxonomy."""
    if require_taxonomy_version is None:
        return True, None
    supported = f"v{VERIFY_REASON_TAXONOMY_VERSION}"
    requested = require_taxonomy_version.strip()
    if requested in {supported, str(VERIFY_REASON_TAXONOMY_VERSION)}:
        return True, None
    return (
        False,
        (f"required taxonomy_version={requested!r} does not match supported taxonomy_version={supported}"),
    )
