...
def _taxonomy_version_reason(
    required_taxonomy_version: int | None,
) -> tuple[bool, str | None]:
    if required_taxonomy_version is None:
        return True, None
    if required_taxonomy_version == VERIFY_REASON_TAXONOMY_VERSION:
        return True, None
    return (
        False,
        (
            f"requested taxonomy_version={required_taxonomy_version!r} "
            f"does not match supported taxonomy_version={VERIFY_REASON_TAXONOMY_VERSION}"
        ),
    )
...
def _verification_reasons(
    *,
    ...
    if not taxonomy_version_ok:
        reasons.append(VERIFY_REASON_TAXONOMY_VERSION_UNSUPPORTED)
    if not policy_ok:
        reasons.append(VERIFY_REASON_STRUCTURE_INVALID)
    if not retention_ok:
        reasons.append(VERIFY_REASON_STRUCTURE_INVALID)
...
def verify_proof_bundle(
    ...
    *,
    ...
    taxonomy_version_ok, taxonomy_version_reason = _taxonomy_version_reason(require_taxonomy_version)
    ...
    elif not metadata_ok or not taxonomy_version_ok:
