...
AnchorVerificationStatus = Literal["verified", "failed", "not_performed", "quarantined"]
...
class AnchorVerificationResult:
    ...
    ``quarantined`` is reserved for live-provider outages: the bundle was
    not accepted as valid, but the verifier also did not observe a
    malformed anchor token.
    """

    quarantine_reason: str | None = None
    """Non-null when ``verification_status == 'quarantined'``."""
...
def verify_chain_with_anchor_provider(
    events: list[ChainedEvent],
    ...
    *,
    ...
    """Live anchoring helper that quarantines provider outages.

    This wrapper is for the nightly live FreeTSA path: it requests a
    timestamp for the current chain tip, then reuses
    :func:`verify_chain_with_anchors` for the cryptographic checks.

    - ``TSAUnavailableError`` becomes ``verification_status='quarantined'``.
    - ``AnchorVerificationError`` remains a fail-closed verification failure.
    - A successful live request returns the same ``AnchorVerificationResult``
      as :func:`verify_chain_with_anchors`.
    """
    chain_result = verify_chain(events)
    if not events:
        return AnchorVerificationResult(
            chain_ok=chain_result.ok,
            chain_reason=chain_result.reason,
            anchored_seqs=frozenset(),
            unanchored_seqs=frozenset(),
            anchor_results=tuple(),
            verification_status="not_performed",
            quarantine_reason=None,
        )

    request = TimestampRequest(digest=events[-1].event_hash)
    try:
        anchor = provider.request_timestamp(
            request,
            anchored_seq=events[-1].seq,  # type: ignore[call-arg]
        ...
        return AnchorVerificationResult(
            ...
            chain_reason=chain_result.reason,
            anchored_seqs=frozenset(),
            unanchored_seqs=frozenset(ev.seq for ev in events),
            anchor_results=tuple(),
    ...
    return AnchorVerificationResult(
        ...
        anchored_seqs=result.anchored_seqs,
        unanchored_seqs=result.unanchored_seqs,
