...
class TestTSAAuthority:
    ...
    def issue_ocsp_response(self, *, gen_time: datetime) -> bytes:
        """Produce a simplified OCSP-shaped response for the leaf cert.

        This is **not** a fully-conformant RFC-6960 OCSP response — it is
        a deterministic byte sequence that mirrors the relevant fields
        (status, this_update, signature) so the verifier can perform
        cross-reference checks. Full OCSP validation arrives in a
        follow-up alongside live TSA integration.
        """
        # The OCSP response is encoded as a CMS-style SignedData wrapping
        # a simple status structure. This keeps the field present in
        # AnchorRecord for CAdES-A "freeze the evidence" requirement
        # without claiming full OCSP conformance.
...
class TestTSAProvider(TSAProvider):
    ...
    def __init__(self, authority: TestTSAAuthority, *, fail_with: Exception | None = None) -> None:
