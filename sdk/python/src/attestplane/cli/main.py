...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_verify = sub.add_parser(
        ...
        epilog=(
            ...
            "violation; 3 quarantined anchor verdict; 1 cryptographic, "
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    if anchor_status == "quarantined" and result.ok:
        human = f"FAIL: quarantined anchor verdict for {bundle_path}"
    if explain:
    ...
    if anchor_status == "quarantined" and result.ok:
        return VERIFY_QUARANTINE_EXIT_CODE
    if result.ok:
        return 0
