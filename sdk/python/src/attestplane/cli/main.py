...
def _parse_taxonomy_version_pin(value: str) -> int:
    text = value.strip()
    if text.startswith("v"):
        text = text[1:]
    if text.isdigit():
        return int(text)
    raise argparse.ArgumentTypeError("expected taxonomy version pin in the form v1 or 1")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_verify.add_argument(
        ...
        type=_parse_taxonomy_version_pin,
        metavar="vN",
        help=("fail closed unless the verifier rejection taxonomy version matches the supplied pin"),
    )
