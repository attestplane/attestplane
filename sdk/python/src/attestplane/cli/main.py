...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_verify.add_argument(
        ...
        action="store_true",
        help="treat quarantined anchoring state as a hard verification failure",
    )
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    strict_quarantine = anchoring_status == "quarantined" and strict_anchoring
    if strict_quarantine:
        human = f"FAIL: anchoring quarantined: {quarantine_reason or 'quarantined anchor'}"
    else:
    ...
    if explain and not args.json_output and (not result.ok or strict_quarantine):
        if strict_quarantine:
            _write_verify_explanations(
                [
                    {
                        "primary_reason": VERIFY_REASON_ANCHOR_INVALID,
                        "pointer": "/anchoring/quarantine_reason",
                        "message": quarantine_reason or "quarantined anchor",
                    }
                ]
            )
        else:
    ...
    if strict_quarantine:
        return 1
    if anchoring_status == "quarantined" and result.ok:
        return 3
    if result.ok:
        return 0
