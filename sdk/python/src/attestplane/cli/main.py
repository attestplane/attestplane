...
def _verify_human_summary(
    ...
    *,
    ...
    anchoring = getattr(result, "anchoring", None)
    if getattr(anchoring, "status", None) == "quarantine":
        cause = getattr(anchoring, "cause", None) or "unspecified"
        return f"{status} {summary} anchoring.status=quarantine anchoring.cause={cause}"
