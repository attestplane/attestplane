...
def _schema_path_from_bundle_error(text: str) -> str:
    ...
    if "chain_metadata.evidence_taxonomy_version" in text:
        return "/chain_metadata/evidence_taxonomy_version"
    ...
    if not result.metadata_ok:
        ...
        if detail is not None:
            if "schema_version is missing" in detail:
                ...
                path = "/chain_metadata/schema_version"
            elif "evidence_taxonomy_version is missing" in detail:
                code = VERIFY_REASON_SCHEMA_VERSION_MISSING
                ...
                path = "/chain_metadata/schema_version"
            elif "evidence_taxonomy_version must be an integer" in detail:
                code = VERIFY_REASON_SCHEMA_INVALID
                path = "/chain_metadata/evidence_taxonomy_version"
            elif detail.startswith("chain_metadata.schema_version="):
                code = VERIFY_REASON_SCHEMA_VERSION_UNSUPPORTED
                path = "/chain_metadata/schema_version"
            elif detail.startswith("chain_metadata.evidence_taxonomy_version="):
                code = VERIFY_REASON_SCHEMA_VERSION_UNSUPPORTED
                path = "/chain_metadata/evidence_taxonomy_version"
