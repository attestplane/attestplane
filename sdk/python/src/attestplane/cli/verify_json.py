...
def _verify_success_summary(
    ...
    *,
    ...
    resolved_taxonomy_version = (
        resolve_verify_reason_taxonomy_version() if taxonomy_version is None else taxonomy_version
    )
    return (
        f"signer_subject={_bundle_signer_subject(bundle)} "
        f"schema_version={_bundle_schema_version(bundle)} "
        f"taxonomy_version={format_verify_reason_taxonomy_version(resolved_taxonomy_version)} "
        ...
        if bundle is None:
            return [
                _explanation_entry(
                    ...
                    f"taxonomy_version={format_verify_reason_taxonomy_version(None)} anchor=unknown",
                )
            ]
        return [
            _explanation_entry(
                None,
                "/",
                _verify_success_summary(bundle, taxonomy_version=getattr(result, "taxonomy_version", None)),
            )
        ]
...
def _json_failure(
    *,
    ...
    payload = {
        ...
        "taxonomy_version": resolve_verify_reason_taxonomy_version(),
        "reasons": [reason],
        "bundle": {
