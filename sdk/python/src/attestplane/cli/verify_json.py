...
VERIFY_JSON_EXIT_CODE_VERIFIED: int = 0
...
VERIFY_JSON_EXIT_CODES: dict[str, int] = {
    "verified": VERIFY_JSON_EXIT_CODE_VERIFIED,
...
def _anchoring_payload(bundle: dict[str, Any] | None, *, exit_code: int) -> dict[str, Any]:
    if exit_code == VERIFY_JSON_EXIT_CODE_PINNING_GATE_FAILURE:
        status = "quarantined"
        quarantined = True
    else:
        explicit = _bundle_explicit_anchoring_state(bundle)
...
def _verify_success_summary(bundle: dict[str, Any], *, result: Any | None = None) -> str:
    taxonomy_version = _verify_taxonomy_version(result)
    return (
...
def _verify_taxonomy_version(result: Any | None = None) -> int:
    if result is None:
        ...
        if bundle is None:
            return [
                _explanation_entry(
                    ...
                    f"taxonomy_version={format_verify_taxonomy_version(taxonomy_version)} anchor=unknown",
                )
            ]
        return [_explanation_entry(None, "/", _verify_success_summary(bundle, result=result))]
