...
| GPG | Public key TBD; will be published at `keys.openpgp.org` and `keyserver.ubuntu.com` by M5 W6 — see `docs/security/gpg-key-rotation-playbook.md` |
...
**Status**: Pending generation (target: M5 W6, 2026-08-15 GA).

**Fingerprint** (when generated): `TBD — see commit landing rotation playbook`

**Public key**: TBD — published at keys.openpgp.org and keyserver.ubuntu.com after generation.

A dedicated GPG key for `security@attestplane.com` will be generated by a
maintainer running
[`scripts/security/generate-security-gpg-key.sh`](scripts/security/generate-security-gpg-key.sh)
on a secure machine, following
[`docs/security/gpg-key-rotation-playbook.md`](docs/security/gpg-key-rotation-playbook.md).
Until that date no project GPG key is in circulation; do not trust any
key claiming to be the `security@attestplane.com` key prior to publication
through the channels listed below.

**Publication channels (all on the same day at key cut):**

- This `SECURITY.md` file (fingerprint inlined above, replacing the `TBD` placeholder).
- `https://keys.openpgp.org/` (canonical, verified UID).
- `https://keyserver.ubuntu.com/` (mirror).
- The project's GitHub Settings → SSH and GPG keys for the maintainer holding the security contact role.
