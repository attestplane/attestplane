...
**GPG key:** Explicitly deferred; private disclosure remains available by email and GitHub Security Advisories. See `docs/security/release-signing.md` for the decision and its consequences.
...
| GPG | Deferred for now; use email or GitHub Security Advisories. See `docs/security/release-signing.md` for the deferral window, blockers, and readiness impact. |
...
## GPG Key for security@attestplane.com

**Status**: Explicitly deferred until the later signing milestone recorded in
[`docs/security/release-signing.md`](docs/security/release-signing.md).

**Why it is deferred**

- The project does not yet have an approved protected-environment custody
  workflow for release-signing material or the dedicated security-contact key.
- A second triage contact has not yet been designated for continuity.
- Private key material must never appear in repository history, issue text, PR
  text, or logs; deferring the key avoids accidental leakage while the custody
  model is still being defined.

**Current reporting path**

- Email: `security@attestplane.com`
- GitHub Security Advisories: use the "Report a vulnerability" button in the
  Security tab for private disclosure with TLS-in-transit confidentiality.

**Impact of the deferral**

- **CNA readiness.** The vulnerability-disclosure process remains usable, but
  CNA / CVE application material must treat the dedicated GPG channel as not
  yet live.
- **Scorecard.** This deferral does not change the current Scorecard posture,
  because Scorecard tracks repository-visible security hygiene rather than a
  private reporting-key rollout.
- **Readiness claims.** Do not claim a published `security@attestplane.com`
  GPG key, encrypted-reporting workflow, or GPG-based release-signing custody
  until the deferral is lifted and the key is actually published.

**When the deferral is lifted**

The maintainer will generate the key on a secure machine with
[`scripts/security/generate-security-gpg-key.sh`](scripts/security/generate-security-gpg-key.sh),
then publish the fingerprint and public key through the channels named in the
rotation playbook. Until then, no fingerprint text should appear in
repository history, issue text, PR text, or logs.
