...
完整的 runbook（数据库、TSA sidecar、本地 Rekor 实例）见 [`docs/runbooks/LOCAL_DEV.md`](docs/runbooks/LOCAL_DEV.md)。
...
scope 对应 crate 或 SDK 包名，例如 `feat(attestchain)`、`fix(python-sdk)`、`docs(adr)`。
...
- 任何 **load-bearing 架构决定**（新框架映射、hash chain 算法变更、TSA 集成方式、安全边界调整）
  必须附带 ADR（Architecture Decision Record），放入 [`docs/adr/`](docs/adr/)。
