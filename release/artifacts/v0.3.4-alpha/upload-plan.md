# v0.3.4-alpha Release-Asset Upload Plan
...
```text
sdk/python/dist/attestplane-0.3.4a0-py3-none-any.whl
sdk/python/dist/attestplane-0.3.4a0.tar.gz
sdk/typescript/attestplane-attestplane-0.3.4-alpha.tgz
release/artifacts/v0.3.4-alpha/checksums.sha256
release/artifacts/v0.3.4-alpha/artifact-manifest.json
```

## Release Commands

```bash
git tag -a v0.3.4-alpha -m "v0.3.4-alpha"
git -c http.version=HTTP/1.1 push origin v0.3.4-alpha
gh release create v0.3.4-alpha --prerelease --title "v0.3.4-alpha" --notes-file docs/release-notes/v0.3.4-alpha.draft.md ...
...
gh workflow run manage-npm.yml -f action=dist-tag-set-latest-to-version -f version=0.3.4-alpha --ref main
```
